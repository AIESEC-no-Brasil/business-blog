 ...
 * (2) Save like data
 */
add_action( 'wp_ajax_nopriv_tp-post-like', 'jm_post_like' );
add_action( 'wp_ajax_tp-post-like', 'jm_post_like' );
...
function getPostLikeLink( $post_id ) {
	...
	$output = '<a href="#" class="tp-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'&nbsp;'.$count.'</a>';
	return $output;
}
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'themepixels' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo wp_kses( $like_list, array( 'p' => array(), 'a' => array( 'href' => array(), 'title' => array() ) ) );
			} else {
				echo "<p>" . _e( 'You don\'t like anything yet.', 'themepixels' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . __( 'You Like:', 'themepixels' ) . "</h3>\n";
			$like_list .= "<ul>\n";
		...
		echo wp_kses( $like_list, array( 'aside' => array(), 'h3' => array(), 'ul' => array(), 'li' => array(), 'a' => array( 'href' => array(), 'title' => array() ) ) );
	}
}
...
function jm_most_popular_today() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'Today\'s Most Popular Posts', 'themepixels' ) . "</h3>\n";
		echo "<ul>\n";
...
function jm_most_popular_month() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Month\'s Most Popular Posts', 'themepixels' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Week\'s Most Popular Posts', 'themepixels' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . _e( 'Most Popular Posts', 'themepixels' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(
