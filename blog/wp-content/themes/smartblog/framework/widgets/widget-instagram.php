					...
					<div class="instagram-slider-widget flexslider">
						<ul class="slides">
							<?php foreach( $media_array as $item ) {
								echo '<li><figure class="instagram-widget-thumbnail-wrapper"><a href="'. esc_url( $item['link'] ) .'" target="_blank"><img src="'. esc_url($item['thumbnail']) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"/></a></figure></li>';
							} ?>
						</ul>
					</div>
				</div>
